...
const DeliverOrderUpdate = ({
  ...
  const increment = (category, index) => {
    setKoiData((prevData) => {
      ...
      // Allow increment only if the total selected quantity is less than koiQuantity
  ...
  const handleSubmit = async () => {
    const totalKoiSelected = Object.values(koiData)
        .flat()
        .reduce((acc, qty) => acc + Number(qty), 0); // Ensure all values are numbers

    if (totalKoiSelected !== koiQuantity) {
      setErrorMessage(
          `Tổng số lượng cá phải là ${koiQuantity}. Hiện tại bạn đã chọn ${totalKoiSelected}.`
      );
      return; // Do not proceed if the quantity is insufficient
    }
    ...
    try {
      ...
      if (response.data.code === 200) {
        onSubmitSuccess(response.data.result); // Call onSubmitSuccess with API data
