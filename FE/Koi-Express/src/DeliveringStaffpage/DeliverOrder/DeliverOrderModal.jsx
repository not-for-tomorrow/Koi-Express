...
import KoiPriceDetailsModal from "./KoiPriceDetailsModal.jsx";
...
const DeliverOrderModal = ({
    ...
    const [showKoiPriceDetails, setShowKoiPriceDetails] = useState(false);
    ...
    const calculateTotalPrice = () => {
        ...
        Object.keys(koiData).forEach((category) => {
            koiData[category].forEach((qty, index) => {
                const size = index === 0 ? 20 : index === 1 ? 40 : 60;
                total += qty * pricePerSize[size];
            });
        });
    ...
    return (
        <div className="relative z-20 flex flex-col w-full h-full max-w-lg p-6 bg-white border border-gray-200 shadow-lg">
                ...
                <div className="p-4 mt-4 border rounded-lg bg-gray-50">
                    <div className="flex justify-between">
                        ...
                        <div className="flex items-center">
                            ...
                            <button
                                onClick={() => setShowKoiPriceDetails(true)} // Show koi details modal
                                className="ml-2 text-gray-600 hover:text-gray-800"
                                title="Xem chi tiết cá koi"
                            >
            ...
            <div
                className="flex items-center justify-between p-2 mt-4 transition duration-300 bg-white rounded-lg shadow-md hover:shadow-lg"
            >
                <div className="flex-grow sm:pr-4">
                    ...
                    >
                        <label className="block text-xs font-medium text-gray-600">Hình thức thanh toán</label>
            ...
            <div className="flex-shrink-0 mt-6">
                <button
                    ...
                    disabled={remainingTransportationFee === 0}
                >
                    Giao Hàng
                </button>
            </div>
            ...
            {/* Koi Details Modal */}
            {showKoiPriceDetails && (
                <KoiPriceDetailsModal
                    koiData={koiData}
                    koiQuantity={koiQuantity}
                    totalPrice={totalPrice} // Passing total price to the modal
                    onClose={() => setShowKoiPriceDetails(false)} // Close koi details modal
                />
            )}
        </div>
    );
