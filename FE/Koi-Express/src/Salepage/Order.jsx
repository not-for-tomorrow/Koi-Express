...
import { getTranslatedStatus, getStatusColor, statusColors } from "/src/koi/utils/statusUtils.js";
...
const Order = () => {
  ...
  const handleApplyFilter = () => {
    ...
    setDisplayDateRange(getDateRangeDisplay(tempSelectedTimeFilter, customDateRange));
  ...
  const filterOrders = () => {
    ...
    if (selectedTab !== "Tất cả") {
      filteredOrders = filteredOrders.filter(
          (orderWrapper) => getTranslatedStatus(orderWrapper.order.status) === selectedTab
      );
    }
                  ...
                  {Object.keys(statusColors).map((tab, index) => (
                      <button
                          ...
                          className={`px-4 py-2 rounded-full transition duration-300 text-sm ${
                              selectedTab === tab ? "font-bold shadow-md bg-blue-100 text-blue-900" : "text-blue-700 bg-transparent"
                          }`}
                          style={{
                  ...
                  <div className="relative">
                    <button
                        ...
                        className={`flex items-center p-2 text-sm transition duration-300 rounded-lg shadow-sm ${
                            isTimeFilterExpanded ? "bg-blue-100 text-blue-900" : "text-blue-700 bg-transparent"
                        }`}
                    >
                ...
                {filterOrders().length === 0 ? (
                    ...
                    <table className="w-full text-sm text-left border-collapse shadow-md table-auto">
                      ...
                      {filterOrders().map((orderWrapper, index) => {
                        ...
                        const statusColor = getStatusColor(getTranslatedStatus(order.status));

                        return (
                            <tr
                                key={index}
                                className="transition duration-300 border-b border-gray-200 cursor-pointer hover:bg-blue-50"
                                onClick={() => navigate(`/salepage/allorder/detail/${order.orderId}`, { state: { orderId: order.orderId } })}
                            >
                              <td className="p-2 font-semibold text-blue-600">{order.orderId}</td>
                              <td className="p-2 text-sm text-gray-700">{customer.fullName}</td>
                              <td className="p-2 text-sm text-gray-700 w-1/4">
                                <div className="tooltip">
                                  {truncateAddress(order.originLocation)}
                                  <span className="tooltip-text">{order.originLocation}</span>
                                </div>
                              </td>
                              <td className="p-2 text-sm text-gray-700 ">
                                <div className="tooltip">
                                  {truncateAddress(order.destinationLocation)}
                                  <span className="tooltip-text">{order.destinationLocation}</span>
                                </div>
                              </td>
                              ...
                              <td className="p-2 text-sm font-medium text-blue-600 text-center align-middle">
                                {order.totalFee !== null ? `₫ ${order.totalFee.toLocaleString("vi-VN")}` : "N/A"}
                              </td>
