  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setIsOtpStage(true); // Move to OTP input stage
      } else {
        ...
        if (token) {
          ...
          if (role === "CUSTOMER") {
            const userInfoResponse = await axios.get(
              "http://localhost:8080/api/customers/basic-info",
              {
            ...
            if (userInfoResponse.status === 200) {
              localStorage.setItem(
                "userInfo",
                JSON.stringify(userInfoResponse.data.result)
              );
            } else {
          ...
          if (role === "CUSTOMER") navigate("/appkoiexpress");
          else setError("Unexpected role detected.");
        } else {
