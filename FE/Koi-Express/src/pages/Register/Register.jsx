...
const Register = () => {
    ...
    return (
        <section className="flex items-center justify-center min-h-screen registerpage bg-gray-50">
            ...
            <div
                className="flex items-center justify-start max-w-3xl p-5 bg-gray-100 shadow-lg registercard rounded-2xl">
                ...
                <div className="px-8 md:w-1/2 md:px-16">
                    ...
                    {!isOtpStage ? (
                        <form onSubmit={handleRegisterSubmit} className="flex flex-col gap-4" autoComplete="off">
                            <input className="p-2 border rounded-xl" type="text" placeholder="Họ tên" value={fullName}
                                   onChange={(e) => setFullName(e.target.value)} required/>
                            <input className="p-2 border rounded-xl" type="email" placeholder="Địa chỉ Email"
                                   value={email} onChange={handleEmailChange} required/>
                            <input className="p-2 border rounded-xl" type="text" placeholder="Số điện thoại"
                                   value={phoneNumber} onChange={handlePhoneNumberChange} required/>
                            <input className="w-full p-2 border rounded-xl" type="password" placeholder="Mật khẩu"
                        ...
                        <form onSubmit={handleOtpSubmit} className="flex flex-col gap-4 mt-4">
                            <p className="mt-2 text-sm text-gray-600">Please enter the 4-digit OTP sent
                                to {phoneNumber}.</p>
                            <input id="otpInput" className="p-2 text-center border rounded-lg" type="text" maxLength="4"
                    ...
                    {!isOtpStage && (
                        <div className="mt-5 text-xs flex justify-between items-center text-[#002D74]">
                            ...
                            <Link to="/login">
                                <button
                                    className="px-5 py-2 duration-300 bg-white border rounded-xl hover:scale-110">Đăng
                                    nhập
                                </button>
                            </Link>
                        </div>
