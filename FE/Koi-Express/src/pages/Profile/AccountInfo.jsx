...
const translations = {
  ...
  vi: {
    ...
    tags: "Tag đơn hàng",
    addNewTag: "Tạo mới",
    enterNewTag: "Nhập tên tag mới:",
    potentialCustomer: "Khách hàng tiềm năng",
    bulkyItem: "Hàng cồng kềnh",
    returnItem: "Trả hàng",
  },
};
...
const AccountInfo = ({ fullName, phoneNumber, email }) => {
  ...
  const [tags, setTags] = useState([
    translations[language].potentialCustomer,
    translations[language].bulkyItem,
    translations[language].returnItem,
  ]);

  const handleLanguageChange = (lang) => {
    setLanguage(lang);
    setTags([
      translations[lang].potentialCustomer,
      translations[lang].bulkyItem,
      translations[lang].returnItem,
    ]);
  };

  const handleAddTag = () => {
    const newTag = prompt(translations[language].enterNewTag);
    if (newTag) {
      setTags([...tags, newTag]);
    }
  };
  ...
  const t = translations[language]; // Shortcut to access current translations
  ...
  return (
      <div className="p-8 mt-4 space-y-6 md:mt-0">
        ...
        <div className="flex items-center justify-between p-6 transition-shadow duration-300 ease-in-out bg-white rounded-lg shadow-md hover:shadow-lg">
          <div>
            <h1 className="text-[20px] font-semibold text-gray-800">{t.language}</h1>
          </div>
          <div className="flex items-center space-x-4">
            <button
                onClick={() => handleLanguageChange("en")}
                className="flex items-center px-4 py-2 text-sm transition-colors duration-300 border border-gray-300 rounded-lg hover:bg-gray-100"
            >
              <img
        ...
        <div className="p-6 transition-shadow duration-300 ease-in-out bg-white rounded-lg shadow-md hover:shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-[20px] font-semibold text-gray-800">{t.tags}</h1>
            <button
